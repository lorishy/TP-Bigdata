...
spark.sql("CREATE DATABASE IF NOT EXISTS messages")


spark.sql("USE messages")

spark.sql("DROP TABLE IF EXISTS messages_data")
...
query = df_kafka.writeStream \
	.outputMode("append") \
	.format("hive") \
	.option("checkpointLocation", "/tmp/messages_data_checkpoint") \
	.start("messages.messages_data")
